...
export class CustomCardDataService {
  ...
  async createCustomCardData(issueNumber: number): Promise<CustomCardMetadataModel|null> {
    ...
    data.customFields.dev = await this.getRedmineUserFromDeveloperField(issueNumber)
  ...
  private async getRedmineUserFromDeveloperField(issueNumber: number): Promise<RedmineUser|null> {
    const issuesLoader = this.getIssuesLoader()
    const issueData = await issuesLoader.getIssueData(issueNumber)
    if (!issueData) {
      return null
    ...
    const userId = issueData.assigned_to?.id
    if (!userId) {
      return null
    }
    return await this.getUsersLoader().getUserInfo(userId)
